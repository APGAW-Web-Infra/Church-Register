...
// boot the database and migrate
//$app->make('Illuminate\Database\Console\Migrations\MigrateCommand')->handle();

$app['db']->connection()->getSchemaBuilder()->dropAllTables();
$app['db']->connection()->getSchemaBuilder()->create('users', function($table){ $table->id(); $table->string('name'); $table->string('email'); $table->timestamp('email_verified_at')->nullable(); $table->string('password'); $table->rememberToken(); $table->timestamps(); });
...
$messages = DirectMessage::query()->where('sender_id', $member->id)->orWhere('recipient_id', $member->id)->with(['sender:id,name', 'recipient:id,name'])->latest()->get();
$threads = $messages->groupBy(fn($message) => $message->sender_id === $member->id ? $message->recipient_id : $message->sender_id)->map(function($thread) use ($member) {
...
})->sortByDesc('sort_stamp')->map(function ($thread) { unset($thread['sort_stamp']); return $thread; })->values();
var_export($threads->toArray());
